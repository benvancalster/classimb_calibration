...
source('Ruben van den Goorbergh/performance_measures bvc nov21.R')
...
# Fit model
rf_model <- randomForest(formula = outcome1~., data = train_set, mtry=1)
#summary(rf_model)
...
rf_pred2<- rep(0, length(rf_probs)) # create vector for class predictions
rf_pred2 [rf_probs > .192] = 1 # Predict classes with threshold 0.5
...
# Fit model
rf_model_down <- randomForest(formula = outcome1~., data = train_down, mtry=1)
...
rf_pred2_down <- rep(0, length(rf_probs_down))# create vector for class predictions
rf_pred2_down[rf_probs_down > .192] = 1 # Predict classes with threshold 0.5
...
# Fit model 
rf_model_up <- randomForest(formula = outcome1~., data = train_up, mtry=1)
#summary(rf_model_up)
...
rf_pred2_up <- rep(0, length(rf_probs_up))# create vector for class predictions
rf_pred2_up[rf_probs_up > .192] = 1 # Predict classes with threshold 0.5
...
rf_model_smote<- randomForest(formula = outcome1~., data = train_smote, mtry=1)
...
rf_pred2_smote <- rep(0, length(rf_probs_smote))# create vector for class predictions
rf_pred2_smote[rf_probs_smote > .192] = 1 # Predict classes with threshold 0.5
...
probs_table <- cbind(log_probs, log_probs_down, log_probs_up, log_probs_smote,
                     ...
                     Winsorize(rf_probs,minval = 0.001, maxval = 0.999), Winsorize(rf_probs_down,minval = 0.001, maxval = 0.999), 
                     Winsorize(rf_probs_up,minval = 0.001, maxval = 0.999), Winsorize(rf_probs_smote,minval = 0.001, maxval = 0.999))
...
pred_table <- cbind(log_pred, log_pred_down, log_pred_up, log_pred_smote,
                    ...
                    (rf_pred), (rf_pred_down), (rf_pred_up), (rf_pred_smote))
pred2_table <- cbind(log_pred2, log_pred2_down, log_pred2_up, log_pred2_smote,
                    ...
                    (rf_pred2), (rf_pred2_down), (rf_pred2_up), (rf_pred2_smote))
...
box(which="plot")
mtext("No adjustment", side=3, line=1.3)
...
box(which="plot")
mtext("LR", side=4, las=1, line=1.3)
mtext("SMOTE", side=3, line=1.3)
...
box(which="plot")
mtext("Ridge", side=4, las=1, line=1.3)
val.prob.ci.2(p = probs_table[,9], y = test_set$outcome1, smooth="loess", legendloc=F, lwd.smooth=2, lwd.ideal=2, lty.ideal=2, dostats = F, axes = F)
axis(side=2,at=c(0,0.2,0.4,0.6,0.8,1))
axis(side=1,at=c(0,0.2,0.4,0.6,0.8,1))
box(which="plot")
val.prob.ci.2(p = probs_table[,10], y = test_set$outcome1, smooth="loess", legendloc=F, lwd.smooth=2, lwd.ideal=2, lty.ideal=2, dostats = F, axes = F)
axis(side=1,at=c(0,0.2,0.4,0.6,0.8,1))
box(which="plot")
val.prob.ci.2(p = probs_table[,11], y = test_set$outcome1, smooth="loess", legendloc=F, lwd.smooth=2, lwd.ideal=2, lty.ideal=2, dostats = F, axes = F)
axis(side=1,at=c(0,0.2,0.4,0.6,0.8,1))
box(which="plot")
val.prob.ci.2(p = probs_table[,12], y = test_set$outcome1, smooth="loess", legendloc=F, lwd.smooth=2, lwd.ideal=2, lty.ideal=2, dostats = F, axes = F)
axis(side=1,at=c(0,0.2,0.4,0.6,0.8,1))
box(which="plot")
mtext("RF", side=4, las=1, line=1.3)
title(xlab = list("Estimated probability",cex=2),
      ylab = list("Observed proportion",cex=2), 
      outer = TRUE)#, line=3)


# using val.prob.ci.2; without RF
par(mfrow = c(4,2),
    oma=c(5,5,2,9) + 0.0,
    mar=c(0,0,1,1) + 0.0)
val.prob.ci.2(p = probs_table[,1], y = test_set$outcome1, smooth="loess", legendloc=F, lwd.smooth=2, lwd.ideal=2, lty.ideal=2, dostats = F, axes = F)
